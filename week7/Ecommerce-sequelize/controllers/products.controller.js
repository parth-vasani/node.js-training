...
async function addProducts(req,res,next){
    ...
    try{
        let result=await Product.create({product_name,product_desc,product_price,sellerId:req.user.id})
...
async function updateProducts(req,res,next){
    ...
    try{
        let [updatedCount]=await Product.update({...dataToUpdate},{where:{id}});
...
async function deleteProducts(req,res,next){
    const {id}=req.params;
    try{
        let deletedCount = await Product.destroy({where:{id}});
