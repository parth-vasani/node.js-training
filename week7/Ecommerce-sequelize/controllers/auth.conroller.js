...
async function registerUser(req, res, next) {
  ...
  try {
    let userExist = await User.findOne({ where: { email }, attributes: ['id','email'] });
    ...
    sendResponse(res, 200, 200, "User registerd", {
...
async function loginUser(req, res, next) {
  ...
  try {
    let user = await User.findOne({ where: { email } ,attributes:['id','email','password']});
    if (!user) {
    ...
    sendResponse(res, 200, 200, "User login", {
      access_token: jwt.sign(payload, jwtSecret, { expiresIn: "1h" }),
    });
  } catch (err) {
    next(err);
